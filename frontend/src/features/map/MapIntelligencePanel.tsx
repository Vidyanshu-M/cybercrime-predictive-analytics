...
export const MapIntelligencePanel: React.FC<MapIntelligencePanelProps> = ({
  ...
  const title = selectedAtm ? selectedAtm.bankName : selectedZone?.name;
  const subtitle = selectedAtm
    ? `${selectedAtm.atmCode} • ${selectedAtm.area}, ${selectedAtm.district}`
    : `Surveillance Zone • ${selectedZone?.radiusKm} km radius`;
  ...
  const handleExportGeoJson = () => {
    const dossier = {
      ...
      properties: {
        ...
        riskLevel,
        riskScore,
        district: selectedAtm?.district || 'Delhi NCR',
        jurisdiction: selectedAtm?.area || selectedZone?.name
      },
      features: [
        {
          type: 'Feature',
          geometry: {
            ...
            coordinates: selectedAtm
              ? [selectedAtm.location.lng, selectedAtm.location.lat]
                ...
                {nearbyComplaints.map((c) => (
                  <div key={c.id} className="bg-cyber-950/80 p-2.5 rounded-xl border border-slate-800 text-xs">
                    <div className="flex items-center justify-between font-mono">
                      <span className="text-purple-300 font-bold">{c.complaintNumber}</span>
                      <span className="text-emerald-400 font-semibold">₹{c.fraudAmount.toLocaleString('en-IN')}</span>
                    </div>
                    <p className="text-[11px] text-slate-300 mt-1">{c.crimeCategory.replace(/_/g, ' ')}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">{c.policeStation}</p>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-1.5 pt-2 border-t border-slate-800">
                ...
                {nearbyTransactions.map((t) => (
                  <div key={t.id} className="bg-cyber-950/80 p-2.5 rounded-xl border border-slate-800 text-xs">
                    <div className="flex items-center justify-between font-mono">
                      <span className="text-amber-300 font-bold">{t.transactionReference}</span>
                      <span className="text-rose-400 font-bold">₹{t.amount.toLocaleString('en-IN')}</span>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-1">
                      Account: {t.accountId} • {new Date(t.transactionTime).toLocaleTimeString()}
                    </p>
                  </div>
                ))}
              </div>

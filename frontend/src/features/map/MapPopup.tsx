...
import { ShieldAlert, AlertTriangle, Clock, MapPin, Building, Activity, FileText, CheckCircle2 } from 'lucide-react';
    ...
    <div className="space-y-4 text-slate-200">
      ...
      <div className="flex items-start justify-between bg-cyber-850 p-4 rounded-xl border border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-bold text-white">{atm.bankName}</h3>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="glass-panel p-3 rounded-xl border border-slate-800">
          <p className="text-[10px] font-bold text-slate-400 uppercase">Predicted Horizon Window</p>
          <p className="text-xs font-semibold text-white mt-1 flex items-center gap-1.5 font-mono">
            <Clock className="w-3.5 h-3.5 text-cyber-accent" />
            <span>Next 3 Hours (07:00 - 10:00)</span>
          </p>
        </div>
        <div className="glass-panel p-3 rounded-xl border border-slate-800">
          <p className="text-[10px] font-bold text-slate-400 uppercase">ML Model Confidence</p>
          <p className="text-xs font-semibold text-emerald-400 mt-1 flex items-center gap-1.5 font-mono">
        ...
        <ul className="space-y-1.5 text-xs text-slate-300">
          {prediction?.reasons ? (
            prediction.reasons.map((r, i) => (
              <li key={i} className="flex items-start gap-2 bg-cyber-950/60 p-2 rounded-lg border border-slate-800/60">
            ...
            <>
              <li className="flex items-start gap-2 bg-cyber-950/60 p-2 rounded-lg border border-slate-800/60">
                <span className="text-cyber-accent font-bold">•</span>
          ...
          <div className="space-y-1.5">
            {nearbyComplaints.slice(0, 2).map((cmp) => (
              <div key={cmp.id} className="glass-card p-2.5 rounded-lg flex items-center justify-between text-xs">
                <div>
                  <p className="font-semibold text-white">{cmp.complaintNumber}</p>
                  <p className="text-[10px] text-slate-400">{cmp.crimeCategory}</p>
                </div>
                <div className="text-right">
                  <p className="text-emerald-400 font-mono font-semibold">₹{cmp.fraudAmount.toLocaleString('en-IN')}</p>
                  <p className="text-[10px] text-slate-400">{cmp.policeStation}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex items-center gap-3 pt-2">
        <button
          ...
          className="flex-1 py-2.5 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 font-bold text-xs uppercase tracking-wider border border-rose-500/40 flex items-center justify-center gap-1.5 transition-colors"
        >

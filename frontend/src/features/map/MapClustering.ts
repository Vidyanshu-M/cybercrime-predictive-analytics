...
export function clusterPoints<T extends ClusterablePoint>(
  ...
  const validPoints = (Array.isArray(points) ? points : []).filter(
    (p) => p && p.location && typeof p.location.lat === 'number' && typeof p.location.lng === 'number' && !isNaN(p.location.lat) && !isNaN(p.location.lng)
  );

  if (zoom >= clusterThresholdZoom || validPoints.length <= 15) {

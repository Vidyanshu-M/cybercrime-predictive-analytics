    ...
    <header className="h-16 border-b border-slate-800/80 bg-cyber-900/80 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            {user?.avatarUrl ? (
              <img src={user.avatarUrl} alt={user?.name || 'User'} className="w-8 h-8 rounded-lg object-cover border border-cyber-accent/40" />
            ) : (
            ...
            <div className="text-left hidden lg:block">
              <p className="text-xs font-semibold text-white leading-tight">{user?.name || 'Officer'}</p>
              <p className="text-[10px] text-cyber-accent font-mono">{String(user?.role || 'I4C_OFFICER').replace('ROLE_', '')}</p>
            </div>
          </button>
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-56 glass-panel bg-cyber-900 border-slate-800 rounded-xl shadow-2xl py-2 z-50 text-xs">
              <div className="px-4 py-2 border-b border-slate-800">
                <p className="font-semibold text-white">{user?.name || 'Officer'}</p>
                <p className="text-slate-400 text-[11px] truncate">{user?.email || 'officer@cybertrace.gov.in'}</p>
                <span className="mt-1 inline-block px-2 py-0.5 rounded bg-cyber-accent/15 text-cyber-accent font-mono text-[10px]">
                  {user?.department || 'Cyber Operations Command'}
                </span>
              </div>

...
import { BarChart3, BrainCircuit, ShieldCheck, Activity, Award, Layers } from 'lucide-react';
...
export const AnalyticsPage: React.FC = () => {
  ...
  if (loading || !metrics || !summary) {
    return <div className="py-20 text-center text-slate-400 text-xs">Loading ML model metrics & charts...</div>;
  }
  ...
  const riskPieData = [
    ...
    { name: 'CRITICAL', value: summary.riskDistribution.critical },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Model Performance Cards Grid (Blueprint Section 8) */}
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={summary.hourlyRiskTrend}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0F1626', borderColor: '#1F2D4A', borderRadius: '12px' }} 
                />
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {summary.topDistrictRisks.map((d, i) => (
            <div key={d.district} className="glass-card p-3.5 rounded-xl border border-slate-800 flex flex-col justify-between space-y-2">
              ...
              <p className="text-xs font-bold text-white">{d.district}</p>

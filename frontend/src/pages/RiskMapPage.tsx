...
export const RiskMapPage: React.FC = () => {
    ...
    async function loadMapData() {
      try {
        const [atmData, cmpData, wthData, zoneData, predData] = await Promise.all([
          ...
          transactionService.getSuspiciousWithdrawals(),
  ...
  const districts = useMemo(() => Array.from(new Set(atms.map((a) => a.district))), [atms]);
  const banks = useMemo(() => Array.from(new Set(atms.map((a) => a.bankName))), [atms]);
            ...
            <div className={selectedAtm || selectedZone ? 'xl:col-span-7' : 'xl:col-span-12'}>
              <ATMMap
                ...
                onComplaintSelect={(cmp) => {
                  const linkedAtm = atms.find((a) => a.district === cmp.district) || atms[0];
                  setSelectedAtm(linkedAtm);
                }}
                onWithdrawalSelect={(txn) => {
                  const linkedAtm = atms.find((a) => a.id === txn.atmId) || atms[0];
                  setSelectedAtm(linkedAtm);
                }}
              />
            </div>
            ...
            {(selectedAtm || selectedZone) && (
              <div className="xl:col-span-5">
                <MapIntelligencePanel
                  ...
                  prediction={selectedAtm ? predictions.find((p) => p.atmId === selectedAtm.id) : null}
                  nearbyComplaints={
                    selectedAtm
                      ? complaints.filter((c) => c.district.toLowerCase() === selectedAtm.district.toLowerCase())
                      : complaints
                  }
                  nearbyTransactions={
                    selectedAtm
                      ? withdrawals.filter((w) => w.atmId === selectedAtm.id)
                      : withdrawals
                  }
                  onClose={() => {
                    setSelectedAtm(null);

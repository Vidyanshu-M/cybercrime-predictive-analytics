...
import { DashboardSummary, ATM, Prediction } from '../types';
...
export const DashboardPage: React.FC = () => {
  ...
  if (loading || !summary) {
    return (
      <div className="py-20 text-center text-slate-500 dark:text-slate-400 text-xs flex flex-col items-center justify-center gap-3">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-5 sm:p-6 rounded-2xl border border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-md text-[11px] font-mono font-bold bg-rose-500/15 text-rose-500 dark:text-rose-400 border border-rose-500/30">
              NATIONAL THREAT LEVEL: HIGH
            </span>
            <span className="text-xs text-slate-500 dark:text-slate-400">Refreshed 2 mins ago</span>
          </div>
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white night:text-white mt-1">
            Command & Control Dashboard
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Geospatial fraud prediction and active cybercrime incident monitoring.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Spatial visualization of active ATM risk pins & NCRP complaint clusters.
              </p>
            </div>
            <button
        ...
        <div className="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white night:text-white flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-rose-500" />
              <span>Pending Command Alerts</span>
            </h3>
            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-rose-500/20 text-rose-500">
          ...
          <div className="space-y-3 overflow-y-auto max-h-[360px] pr-1">
            {summary.recentAlerts.map((alt) => (
              <div
                ...
                className="glass-card p-3 rounded-xl border-l-4 border-l-rose-500 hover:border-pink-300 dark:hover:border-slate-700 cursor-pointer transition-all space-y-1.5"
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400">{alt.district}</span>
                  <RiskBadge level={alt.riskLevel} score={alt.riskScore} size="sm" />
                </div>
                <p className="text-xs font-semibold text-slate-900 dark:text-white line-clamp-2">{alt.message}</p>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-cyber-850 hover:bg-rose-100/60 dark:hover:bg-cyber-800 night:hover:bg-slate-800 text-slate-800 dark:text-slate-300 night:text-slate-300 font-semibold text-xs border border-slate-800 text-center transition-colors"
          >
      ...
      <div className="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Latest cyber fraud reports linked with spatial coordinates.
            </p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead className="bg-cyber-850 border-b border-slate-800 text-slate-500 dark:text-slate-400 font-semibold uppercase text-[10px]">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/60 font-sans">
              {summary.recentComplaints.map((cmp) => (
                <tr key={cmp.id} className="hover:bg-rose-50/60 dark:hover:bg-cyber-800/40 night:hover:bg-slate-800/40 transition-colors">
                  <td className="px-4 py-3 font-mono font-semibold text-cyber-accent">{cmp.complaintNumber}</td>
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{cmp.crimeCategory.replace(/_/g, ' ')}</td>
                  <td className="px-4 py-3 font-mono font-semibold text-emerald-600 dark:text-emerald-400">₹{cmp.fraudAmount.toLocaleString('en-IN')}</td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{cmp.policeStation}</td>
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{cmp.district}</td>
        ...
        {!latestPrediction ? (
          <form onSubmit={handleRunPrediction} className="space-y-4">
            ...
            <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl bg-cyber-accent text-slate-950 font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 shadow-md"
              >
          ...
          <div className="space-y-4">
            ...
            <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500 dark:text-slate-400">Target ATM: <strong className="text-slate-900 dark:text-white">{latestPrediction.atmCode}</strong></span>
                <RiskBadge level={latestPrediction.riskLevel} score={latestPrediction.riskScore} size="lg" />
              </div>

              <div className="bg-cyber-850 p-3 rounded-lg border border-slate-800 text-xs space-y-1">
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-cyber-accent text-slate-950 font-bold text-xs uppercase tracking-wider shadow-md"
            >

...
import { ArrowLeft, FileText, Upload, HardDrive } from 'lucide-react';
...
export const CaseDetailPage: React.FC = () => {
  ...
  if (loading || !caseItem) {
    return <div className="py-20 text-center text-slate-500 dark:text-slate-400 text-xs">Loading case file...</div>;
  }
      ...
      <div className="glass-panel p-5 sm:p-6 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white night:text-white mt-1">{caseItem.title}</h2>
          </div>

          <button
            onClick={() => setUploadModalOpen(true)}
            className="py-2 px-4 rounded-xl bg-cyber-accent text-slate-950 font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 shadow-md"
          >
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300">{caseItem.summary}</p>
      </div>
            ...
            {caseItem.evidenceList.map((evd) => (
              <div key={evd.id} className="glass-card p-3 rounded-xl flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-white">{evd.fileName}</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono truncate max-w-md">SHA256: {evd.hash}</p>
                  </div>
                </div>

                <div className="text-right text-[10px]">
                  <p className="text-purple-600 dark:text-purple-400 font-medium">Uploaded by {evd.uploadedBy}</p>
                  <p className="text-slate-500 dark:text-slate-400">{new Date(evd.uploadedAt).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-xs text-slate-500 dark:text-slate-400">No evidence artifacts uploaded yet.</p>
        )}
      </div>

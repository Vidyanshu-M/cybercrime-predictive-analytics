...
public class CaseService {
    ...
    @Transactional
    public CaseResponse updateCaseStatus(UUID id, String status, User currentUser, String ipAddress) {
        CaseEntity c = caseRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Case not found with id: " + id));

        c.setStatus(status.toUpperCase());
        CaseEntity updated = caseRepository.save(c);

        auditService.logAction(currentUser, "UPDATE_CASE_STATUS", "CASE", id.toString(), ipAddress);

        return mapper.toCaseResponse(updated);
    }
}
